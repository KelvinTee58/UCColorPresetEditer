  ...
  <div class="super-vue3-tabs">
    <div class="flex gap-2 border-b border-border">
      ...
      <div class="flex gap-3 select-none relative overflow-x-auto tabs" ref="tabsContainerRef">
        ...
        <div
          class="underline-marker absolute bottom-0 left-0 h-0.5 rounded origin-left duration-300 ease-in-out transition-[left] hover:tabActive active:tabActive"
          :style="{
        ...
        <DropdownMenu>
          <DropdownMenuTrigger as-child>
            <Button variant="ghost" class="text-sm font-medium px-2 py-2">
              <Icon icon="radix-icons:dots-vertical" class="h-[1.2rem] w-[1.2rem] transition-all" />
            </Button>
          </DropdownMenuTrigger>
          <DropdownMenuContent align="end">
            <DropdownMenuItem v-for="tab in dropwDownList" :key="tab.id" @click="selectTab({ tab })">
              <component v-if="tab.icon" v-for="(node, index) in tab.icon()" :key="index" :is="node" />
              {{ tab.value }}
            </DropdownMenuItem>
          </DropdownMenuContent>
