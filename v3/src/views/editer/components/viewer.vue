...
export default {
  props: {
    scaleTime: { type: Number, default: 0.1 },
  ...
  setup(props, { emit, expose }) {
    const el = shallowRef(null),
      ...
      origin = {
        ...
        rectY: 0, // 容器相对视口的起始坐标
        centerX: 0,
        centerY: 0, // 两点中心相对视口的起始坐标
        offsetX: 0,
        offsetY: 0, // 两点中心相对移动目标左上角的坐标，即相对容器坐标减去translate偏移距离
        length: 0, // 两点起始长度
      },
      setCentre = (touches) => {
    ...
    return {
      ...
      computedTransform: computed(() =>
        Object.entries(props.transform)
      ...
      onWheel(e) {
        ...
        if (e.deltaY > 0) {
          ...
          props.transform.scale -= props.scaleTime || 0.1;
        ...
        switch (e.touches.length) {
          ...
          case 2:
            ...
            const onMove_2 = (eve) => {
                ...
                let scale = (origin.scale * Math.sqrt(Math.pow(eve.touches[0].clientX - eve.touches[1].clientX, 2) + Math.pow(eve.touches[0].clientY - eve.touches[1].clientY, 2))) / origin.length;
